...
int IncluiFuncionario (TDescr *lst, int num){
	if (lst->lista->final == QTDE_FUNC)
		return FALSE;
	else{
		lst->lista[++(lst->lista->final)].codigo = num;
	
		printf("Informe o nome do funcionario\n");
		scanf("%s", lst->lista[(lst->lista->final)].nome);

		printf("Informe o salario do funcionario\n");
		scanf("%f", &lst->lista[(lst->lista->final)].salario);
	return TRUE;
	}
}
...
void ImprimeLista(TDescr *lst, char *cabec){
	...
	if (lst->lista->final == -1)
		puts("Lista esta vazia.");
	else{
		printf("\n\n%s\n", cabec);

		while (p <= lst->lista->final){
			printf("Codigo: %d\nNome: %s\nSalario R$ %.2f\n\n", 
			lst->lista[p].codigo,
			lst->lista[p].nome,
			lst->lista[p].salario);
...
int ExcluiFuncionario(TDescr *lst, int num){
	...
	while (p <= lst->lista->final && lst->lista[p].codigo != num){
		p = p + 1;
	}

	if (p > lst->lista->final)
		return FALSE;
	else{
		while (p < lst->lista->final){
		...
		lst->lista->final = lst->lista->final -1;
		return TRUE;
	}
}